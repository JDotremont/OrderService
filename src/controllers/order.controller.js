...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    for (let orderLineData of req.body.orderLine) {
      const product = await Product.findByPk(orderLineData.productId);
      if (product.qty < orderLineData.qty) {
        throw new Error('Not enough stock');
      }
      if (product.qty - orderLineData.qty < product.alert) {
        const mailOptions = {
